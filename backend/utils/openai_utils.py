...
def get_gpt_extraction(prompt: str, input: str):
    ...
    return "This is a placeholder for the GPT-4 API call"
...
def get_claude_extraction(prompt: str, input: str):
    ...
    return "This is a placeholder for the Claude API call"
import google.generativeai as genai
import os

genai.configure(api_key=config("GOOGLE_API_KEY"))

async def get_palm_extraction(prompt: str, input: str):
    # response = palm.chat(
    #     model="models/chat-bison-001",
    # )
    print("nferneelernfberjfnjxjdekfnfjcecf")
    messages=[
    ...
    # print("debug-3",response.last)
    return [response.text]  # `last` contains the assistant's reply
    # return "This is a placeholder for the Palm API call"
...
def get_llama_extraction(prompt: str, input: str):
    ...
    # )
    # return response.choices[0].message.content
    return "This is a placeholder for the LLaMA API call"
...
async def get_all_extractions(prompt: str, input: str) -> List[str]:
    ...
    print("_______________________________--------------------------------_____________")
    async def get_all_extractions_async(prompt: str, input: str):
        tasks = [
            # get_extraction(get_gpt_extraction, prompt, input),   # GPT extraction
            # get_extraction(get_claude_extraction, prompt, input),  # Claude extraction (Anthropic)
            # get_extraction(get_llama_extraction, prompt, input),   # Groq Llama extraction
            get_extraction(get_palm_extraction,prompt,input),
        ]
        print(str(tasks[0]))
        return await asyncio.gather(*tasks)
    # return await get_palm_extraction(prompt=prompt,input=input)
