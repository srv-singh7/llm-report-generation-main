...
def _get_sales_revenue(
    ...
    print("revenueeeeeeeeeeeeeeeeeeeee",revenue)
    return revenue
...
async def _get_executive_summary(month_num: int, year: int, department: str, company: str = "comp1") -> str:
    ...
    response = await asyncio.gather(*response)  # Collect actual results
    # return [r.strip() for r in response]
    print("i am debuginh page 2 " ,response)
...
async def _get_fin_perf_bullets(month_num: int, year: int, company: str = "comp1") -> str:
    ...
    response = await asyncio.gather(*response)  # Collect actual results
    print(response)
    bullets = [[bullet.strip() for bullet in r.split("|")] for r in response[0]]
    # bullets = [r for r in response]
    return bullets
...
async def _get_product_sales_bullets(
    ...
    response = await asyncio.gather(*response)  # Collect actual results
    return [[bullet.strip() for bullet in r.split("|")] for r in response[0]]
...
async def _get_recommendations_intro(
    ...
    # )
    response = await get_palm_extraction(
        recommendations_prompt, context
    )
    print(response)
    return [r.strip() for r in response]
